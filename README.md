...
---
## Repository has two Versions — Which One to Use?

This repository contains two versions of the same project. Both do exactly the same thing. The only difference is which AI model generates the answer and where that model runs.

---

### Version 1 — Ollama (Fully Offline)
**File:** `eagle_trans_ai_assistant_v1.zip`

Everything runs on your laptop. No internet needed after setup. No data ever leaves your machine.

- Language model: **Llama 3.2** — runs locally via Ollama on your laptop CPU
- Embedding model: **nomic-embed-text** — runs locally via Ollama
- Speed: 10 to 20 seconds per answer
- Data privacy: 100% — nothing leaves the laptop
- Cost: Zero — no API, no subscription
- Best for: Real company deployment where data must stay internal

**Use this if:** You are working with real sensitive data or you want a fully offline setup.

---

### Version 2 — Groq API (Faster)
**File:** `eagle_trans_ai_assistant_v2.zip`

Document search still runs locally. But the answer is generated by a larger, more powerful model running on Groq's servers — which makes it much faster and more accurate.

- Language model: **openai/gpt-oss-20b** — runs on Groq's servers via free API
- Embedding model: **nomic-embed-text** — still runs locally via Ollama
- Speed: 1 to 3 seconds per answer
- Data privacy: Question and retrieved chunks are sent to Groq over the internet
- Cost: Zero — Groq free tier (14,400 requests per day)
- Best for: Demos, development, and testing with dummy data

**Use this if:** You are running a demo or prototype with non-sensitive dummy data and want fast responses.

---

### Quick Comparison

| | Version 1 (Ollama) | Version 2 (Groq) |
|---|---|---|
| ZIP file | eagle_trans_ai_assistant_v1.zip | eagle_trans_ai_assistant_v2.zip |
| Language model | Llama 3.2 (3b) | openai/gpt-oss-20b |
| Model runs on | Your laptop CPU | Groq's servers (USA) |
| Embedding model | nomic-embed-text (local) | nomic-embed-text (local) |
| Internet needed | No — fully offline | Yes — for answer generation |
| Data leaves laptop | Never | Question + chunks sent to Groq |
| Speed | 10 to 20 seconds | 1 to 3 seconds |
| Answer quality | Good | Better (larger model) |
| Groq API key needed | No | Yes — free at console.groq.com |
| Cost | Zero | Zero |
| Port | 8501 | 8502 |
| Best for | Production with real data | Demo and development |

---

> **Recommendation:** Start with Version 2 for the demo. If the company wants to deploy it with real data, switch to Version 1 on an in-house server.
